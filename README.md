...
| `netlify/functions/compare.mjs` | Goggles comparisons: the same question through Web Search or LLM Context, with and without the Goggle |
...
Goggles work with the Web Search, LLM Context, and News Search APIs, not the Answers API. Under each answer, two buttons compare sources with and without the Goggle: **Goggles on Web Search** (ranked results) and **Goggles on LLM Context** (the exact pages the LLM Context + Claude engine reads, using the same freshness window). Each runs two calls with the shopper's exact question, so the only difference between the columns is the Goggle. The comparison is separate from the answer engine.
